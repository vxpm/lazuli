...
fn worker(runner_state: Arc<Shared>) {
    ...
    loop {
        ...
        if executed.hit_breakpoint {
            runner_state.hit_breakpoint.store(true, Ordering::SeqCst);
...
impl Runner {
    pub fn new(lazuli: Lazuli) -> Self {
        let state = Shared {
            ...
            hit_breakpoint: AtomicBool::new(false),
        };
    ...
    pub fn start(&mut self) -> bool {
        self.shared.advance.swap(true, Ordering::SeqCst)
    }
    ...
    pub fn hit_breakpoint(&mut self) -> bool {
        self.shared.hit_breakpoint.swap(false, Ordering::SeqCst)
    }
