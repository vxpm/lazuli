...
impl BlockBuilder<'_> {
    ...
    fn jump(
        ...
        let reason = ExitReason::from_branch(
            BranchMeta::default()
        ...
        let current_pc = self.bd.ins().uextend(ir::types::I64, current_pc);
    ...
    pub fn bc(&mut self, ins: Ins) -> InstructionInfo {
        self.branch(ins, !ins.field_aa(), false, ins.field_bd() as i32)
    }

    pub fn bclr(&mut self, ins: Ins) -> InstructionInfo {
        let lr = self.get(SPR::LR);
        self.branch(ins, false, true, lr)
    }

    pub fn bcctr(&mut self, ins: Ins) -> InstructionInfo {
        let ctr = self.get(SPR::CTR);
        self.branch(ins, false, true, ctr)
    }
}
